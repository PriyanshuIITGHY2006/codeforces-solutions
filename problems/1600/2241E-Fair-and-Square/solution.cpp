...
void solve()
{
    ...
    auto dfs1 = [&](auto &&self, int u, int p) -> void
    {
        ...
        {
            if (((int)sqrt(a[u - 1] * a[u - 1] * a[u - 1])) * ((int)sqrt(a[u - 1] * a[u - 1] * a[u - 1])) == a[u - 1] * a[u - 1] * a[u - 1])
            {
                int sumi = accumulate(all(temp), 0ll);
                for (auto it : temp)
                {
                    total = total + ((sumi - it) * it);
                    sumi -= it;
                }
            }
        }
        else
        {
            if (((int)sqrt(a[u - 1] * a[u - 1] * a[u - 1])) * ((int)sqrt(a[u - 1] * a[u - 1] * a[u - 1])) == a[u - 1] * a[u - 1] * a[u - 1])
            {
                ...
                sumi = accumulate(all(temp), 0ll);
                int squares = 0;
                int cubes = 0;
                rep(i, 0, sz(temp))
                {
                    squares = (squares + (temp[i] * temp[i]));
                    cubes = (cubes + (temp[i] * temp[i] * temp[i]));
                }
                int addi = (((((sumi * sumi)) * sumi) - (3 * ((sumi * squares)))) + ((2 * cubes)));
                addi = (addi/6);
                total = (total + addi);
            }
        }
