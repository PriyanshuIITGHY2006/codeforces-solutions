...
void solve()
{
    ...
    rep(i, 0, n - 1)
    {
        ...
        if(!tiki)
        {
            if(a[i].length()>a[i+1].length()) smexy = true;
        }
        // if (a[i][j] != a[i + 1][j])
        // {
        //     adj[a[i][j] - 'a' + 1].pb(a[i + 1][j] - 'a' + 1);
    ...
    vector<bool> inStack(27, false);
    auto checkCycle = [&](int node, auto &&self) -> bool
    {
        visited[node] = true;
        inStack[node] = true;
        ...
        for (int child : adj[node])
        {
            if (inStack[child])
                return true;
            if (!visited[child] && self(child, self))
                return true;
        }
