...
void solve()
{
    int a, b;
    r(a, b);
    string s1 = to_binary64(a);
    string s2 = to_binary64(b);
    int last_bit_set = -1, last_bit_unset = 65;
    for (int i = 0; i < 64; i++)
    {
        if (s1[i] == '1' && s2[i] == '0')
        {
            last_bit_unset = min(last_bit_unset, i);
        }
        else if (s1[i] == '1' && s2[i] == '1')
        {
            last_bit_set = max(last_bit_set, i);
        }
    ...
    if (last_bit_set != -1 && last_bit_unset != 65)
    {
        if (last_bit_set > last_bit_unset)
        {
            o(-1);
            return;
        }
        bool all1 = true;
        for (int i = last_bit_set; i < last_bit_unset; i++)
        {
            if (s1[i] == '0')
            {
                all1 = false;
    ...
    {
        int index = last_bit_unset;
        for (int i = last_bit_unset; i >= 0; i--)
        {
            if (s1[i] == '1')
                s1[i] = '0';
            else
        ...
        for(int i=last_bit_unset; i<64; i++) s1[i]='0';
        ll x = (ll)stoull(s1, nullptr, 2);
        o(x);
    }
}
