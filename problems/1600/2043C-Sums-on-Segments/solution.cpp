...
pair<int, int> doitmf(vi & a)
{
    ...
    vi b(n+1, 0);
        ...
        rep(i, 0, n+1)
        {
            largest = max(largest, b[i]-mini);
            smallest = min(smallest, b[i]-maxi);
            mini = min(mini, b[i]);
            maxi = max(maxi, b[i]);
        }
...
void solve() {
    ...
    if(location ==-1)
    {
        auto [smallest, largest] = doitmf(a);
        rep(i, smallest, largest+1)
        {
            s.insert(i);
        }
    }
    else
    {
        ...
        auto [u, v] = doitmf(c); auto [x,y] = doitmf(d);
        rep(i, u, v+1)
        {
            s.insert(i);
        }
        ...
        int smallest = cmini + dmini + a[location];
        int largest = cmaxi + dmaxi + a[location];
        rep(i, smallest, largest+1)
        {
            s.insert(i);
        }
