...
void solve(vll &dp, vll &dp1) {
    ...
    {
        int d=s[i]-'0';
        ans += (d*dp[len-i-1]) + (pow(10, len- i -1)*((d*(d-1)/2)+temp*(d)));
        // o(ans
        if(i==len-1) ans+=d+temp;
        temp+=d;
    }
    // o(ans);
    ll temp2= k%base;
...
int main() {
    ...
    for(int i=2; i<=16; i++)
    {
        dp1.pb(pow(10,i-1)*i*9 + dp1[i-1]);
        // o(dp1[i]);
    }
    while (t--) solve(dp, dp1);
    return 0;
}