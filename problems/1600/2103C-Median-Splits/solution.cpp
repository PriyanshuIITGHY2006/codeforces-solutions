...
void solve1()
{
    ...
    rep(i, 0, n)
    {
        if(c[i] <= 0) indexes.pb(i);
    }
    
    per(i, 0, n)
    {
        if(d[i] <= 0) indexes1.pb(i);
    }
    
    if(sz(indexes) && sz(indexes1))
    {
        if(indexes[0] < indexes1[0] - 1)
        {
            YES return;
        }
    }
    ...
    if(sz(indexes) >= 2)
    {
        ...
        rep(i, 1, sz(indexes))
        {
            ...
            maxi = max(maxi, c[indexes[i]]); 
        }
    }
    if(sz(indexes1) >= 2)
    {
        int maxi = d[indexes1[0]];
        rep(i, 1, sz(indexes1))
        {
            if(indexes1[i] != 0)
            {
                if(d[indexes1[i]] <= maxi)
                {
                    YES return;
                }
            }
            maxi = max(maxi, d[indexes1[i]]);
        }
    }
    NO
}
