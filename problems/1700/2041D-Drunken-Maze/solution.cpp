...
void solve() {
    ...
    rep(i, 0, n) rep(j, 0, m) {
        ...
        if (grid[i][j] == 'T') tgt = mp(i, j);
    }
    vector<vector<vvi>> dist(n, vector<vvi>(m, vvi(4, vi(4, INF))));
    priority_queue<tuple<int,int,int,int,int>, vector<tuple<int,int,int,int,int>>, greater<>> q;
    rep(i, 0, 4) {
        dist[start.fi][start.se][i][0] = 0;
        q.push({0, start.fi, start.se, i, 0});
    }
    while (!q.empty()) {
        auto [w, x, y, pd, e] = q.top(); q.pop();
        if (w > dist[x][y][pd][e]) continue;
        rep(d, 0, 4) {
            if (d == pd && e == 3) continue;
            int nx = x + dx[d], ny = y + dy[d];
            int ne = (d == pd) ? e + 1 : 1;
            if (valid(nx, ny) && grid[nx][ny] != '#' && dist[nx][ny][d][ne] > w + 1) {
                dist[nx][ny][d][ne] = w + 1;
                q.push({w + 1, nx, ny, d, ne});
            }
        }
    }
    int ans = INF;
    rep(d, 0, 4) rep(k, 0, 4) ans = min(ans, dist[tgt.fi][tgt.se][d][k]);
    o(ans == INF ? -1 : ans);
}
