...
#define YES cout << "YES\n";
#define NO cout << "NO\n";
bool check(int mid, vvi & a, int n, int m)
{
    vvll k(n+1, vll(m+1, 0));
    
    for(int i=1; i<n+1; i++) {
        for(int j=1; j<m+1; j++) {
            k[i][j] = k[i-1][j] + k[i][j-1] - k[i-1][j-1] + ((a[i-1][j-1]>=mid) ? 1 : 0);
        }
    }
    ...
    for(int i=mid; i<n+1; i++) {
        for(int j=mid; j<m+1; j++) {
            ll ssum = k[i][j] - k[i-mid][j] - k[i][j-mid] + k[i-mid][j-mid];
            if (ssum >= (1LL * mid * mid)) {
                possible = true;
            }
        }
