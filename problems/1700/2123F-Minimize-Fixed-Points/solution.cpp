...
void solve() {
    ...
    for(int i=2; i<=n; i++)
    {
        ...
        {
            bool possible=false;
            vll divisors = prime_factorization(i);
    ...
    for(auto it:m)
    {
        if(it.second.size()==0) answer[it.first-1]=it.first;
        else
        {
            answer[it.first-1]=it.second[0];
            for(int i=0; i<it.second.size()-1; i++)
            {
                answer[it.second[i]-1]=it.second[i+1];
            }
            answer[it.second.back() - 1] = it.first;
        }
    }
