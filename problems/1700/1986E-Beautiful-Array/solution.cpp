...
void solve()
{
    ...
    for (auto it : m)
    {
        ...
        else
        {
            ...
            vi diff1;
            vi diff2;
            for (int i = 0; i < sz(it.se) - 1; i += 2)
            {
                if (sz(diff1) == 0)
                    diff1.pb(temp[i + 1] - temp[i]);
                else
                    diff1.pb(diff1.back() + temp[i + 1] - temp[i]);
            }
            for (int i = sz(it.se) - 1; i > 0; i -= 2)
            {
                if (sz(diff2) == 0)
                    diff2.pb(temp[i] - temp[i-1]);
                else
                    diff2.pb(diff2.back() + temp[i] - temp[i - 1]);
            }
            rev(diff2);
            int ansi = INF;
            rep(i, 0, sz(it.se))
            {
                ...
                {
                    int ti = 0;
                    if(i!=0) ti += diff1[(i/2)-1];
                    if(i!=sz(it.se)-1) ti += diff2[(i/2)];
                    ansi = min(ansi, ti);
                }
            }
            cost += ansi;
