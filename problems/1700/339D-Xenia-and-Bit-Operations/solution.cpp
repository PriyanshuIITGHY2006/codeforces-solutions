...
struct segtree
{
    int n;
    vll tree;
    segtree(int n) : n(n), tree(4 * (1 << n), 0) {}
    int g;
    void setg()
    {
        if (n % 2 == 1)
    ...
    void build(vll &a, int node, int l, int r, int k)
    {
        if (l == r)
        {
            tree[node] = a[l];
        ...
        int mid = (l + r) / 2;
        build(a, 2 * node, l, mid, 1 - k);
        build(a, 2 * node + 1, mid + 1, r, 1 - k);
        if (k == 1)
    ...
    void update(int node, int l, int r, int val, int index, int k)
    {
        if (l == r)
        {
            tree[node] = val;
        ...
        if (index > mid)
        {
            update(2 * node + 1, mid + 1, r, val, index, 1 - k);
        }
        else
        {
            update(2 * node, l, mid, val, index, 1 - k);
        }
        if (k == 1)
            tree[node] = tree[2 * node] | tree[2 * node + 1];
        else
    ...
    void update(int index, ll val) { update(1, 0, (1 << n) - 1, val, index, g); }
};
void solve()
{
    int n, q; r(n, q);
