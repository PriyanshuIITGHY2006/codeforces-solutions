...
#define NO cout<< "NO\n";
void solve() {
    int n; r(n); vi par = rvec(n-1);
    vi ghochu(n+1); rep(i, 1, n+1) r(ghochu[i]);
    vvi adj(n+1);
    rep(i, 0, n-1){
        adj[i+2].pb(par[i]);
        adj[par[i]].pb(i+2);
    ...
    auto dfs = [&](auto && self, int u, int p)->void{
        ...
        if(ghochu[u]!=0){
            ...
            int index = 0;
            rep(i, 1, sz(hulle)){
                if((hulle[i].fi >= hulle[i-1].se) && (hulle[i].se>hulle[i-1].se)) index++;
                else break;
            }
            rep(i, index+1, sz(hulle)) huye.pb(hulle[i]);
            rep(i, 0, min(sz(hulle), index+1)) huye.pb(hulle[i]);
            srt(hulle);
            rep(i, 1, sz(huye)){
                if((huye[i].fi >= huye[i-1].se)) continue;
                else possible = false;;
            }
            hazo[u].fi = huye[0].fi; hazo[u].se = huye.back().se;
        }
    };
    dfs(dfs, 1, 0);
    if(possible) YES
