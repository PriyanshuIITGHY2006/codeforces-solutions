...
void solve()
{
    ...
    per(i, 0, n)
    {
        if (las[s[i]].empty())
            ...
            las[s[i]].pb(las[s[i]].back() + n - i);
    }
    for(auto it = las.begin(); it!=las.end(); it++)
    {
        rev((*it).se);
    }
    dbgm(m); dbgm(fir); dbgm(las);
    __int128_t answer = 0; 
    rep(i, 0, n)
    {
        if (sz(m[t[i]]) == 0)
            continue;
        auto it = lower_bound(all(m[t[i]]), i);
        if (it != m[t[i]].begin())
        {
            it--;
            auto g = it - m[t[i]].begin();
            answer += (__int128_t)(n - i) * fir[t[i]][g];
            it++;
            if (g + 1 < sz(m[t[i]]))
            {
                answer += (__int128_t)(i + 1) * las[t[i]][g + 1];
            }
        }
        else
        {
            int g = it - m[t[i]].begin();
            if (it != m[t[i]].end())
            {
                answer += (__int128_t)(i + 1) * las[t[i]][g];
            }
        }
    }
    int hulle = 0;
