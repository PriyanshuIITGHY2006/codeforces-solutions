...
void solve() {
    ...
    int farthest_node=1;
    int max_dist =0;
    auto dfs = [&](auto& self, int u, int parent, int dist) -> void {
        if (dist > max_dist) {
            max_dist = dist;
            farthest_node = u;
        }
    ...
    dfs(dfs, w, -1, 0);
    int x = farthest_node;
    vi distfromw(n+1,0), distfromx(n+1, 0);
    auto dfsw = [&](auto& self, int u, int parent) -> void {
        if(parent!=-1) distfromw[u]=distfromw[parent]+1;
    ...
    dfsw(dfsw, w, -1);
    auto dfsx = [&](auto& self, int u, int parent) -> void {
        if(parent!=-1) distfromx[u]=distfromx[parent]+1;
    ...
    for(int i=1; i<n+1; i++)
    {
        if(i!=x && i!=w)
        {
            if(((distfromw[i]+distfromx[i])-distfromw[x])/2 >= maxd)
            {
                maxd = ((distfromw[i]+distfromx[i])-distfromw[x])/2;
                node = i;
            }
        }
    }
