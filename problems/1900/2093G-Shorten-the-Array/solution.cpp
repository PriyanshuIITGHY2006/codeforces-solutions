...
void add(string &s, trienode *root)
{
    ...
    rep(i, 0, 31)
    {
        trienode *&nxt = (s[i] == '1' ? cur->right : cur->left);
        if (nxt == nullptr)
            nxt = new trienode();
        cur = nxt;
        cur->val++;
...
void del(string &s, trienode *root)
{
    trienode *cur = root;
    rep(i, 0, 31)
    {
        cur = (s[i] == '1' ? cur->right : cur->left);
        cur->val--;
    }
}
void solve()
{
    int n, k;
    r(n, k);
    vi a = rvec(n);  if(k==0){o(1); return;}
    trienode root{};
    auto live = [&](trienode *p) { return p != nullptr && p->val > 0; };
    auto query = [&](string &s) -> string
    {
        trienode *cur = &root;
        if (!live(cur->left) && !live(cur->right)) return "";
        string ref = "";
        rep(i, 0, 31)
        {
            trienode *want = (s[i] == '0' ? cur->right : cur->left);
            trienode *alt = (s[i] == '0' ? cur->left : cur->right);
            if (live(want)) { ref.pb(char(s[i] ^ 1)); cur = want; }
            else { ref.pb(s[i]); cur = alt; }
        }
        return ref;
    };
    string zero = to_binary(a[0], 31);
    add(zero, &root);
    int li = 0, ri = 1, ans = INF;
    while (ri < n && li <= ri)
    {
        string s = to_binary(a[ri], 31);
        string res = query(s);
        if (sz(res) && (from_binary(res) ^ a[ri]) >= k)
        {
            ans = min(ans, ri - li + 1);
            if (li == ri) { li++; ri++; continue; }
            string s1 = to_binary(a[li], 31);
            del(s1, &root);
            li++;
            continue;
        }
