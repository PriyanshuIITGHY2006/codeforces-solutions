...
void solve()
{
    ...
    for (int i = 2; i < n + 1; i++)
    {
        int parent, weight;
        r(parent, weight);
        adj[parent].pb({i, weight});
    }
    set<pii> q;
    vi dist(n + 1, 0), dummy(n + 1, 0);
    auto dfs = [&](auto &&self, int u) -> void
    {
        int k = - a[u-1] + dist[u];
        if (!q.empty())
        {
            auto it = q.lower_bound({k, 0});
            if (it != q.end())
            {
                if(it!=q.begin()) {it--; dummy[(*it).se]--;}
                dummy[(*prev(q.end())).se]++;
            }
        }
        q.insert({dist[u], u});
    ...
    dfs(dfs, 1);
    vi answer(n + 1, 0);
    auto dfs1 = [&](auto &&self, int u, int k) -> void {
        for (auto it : adj[u])
        {
            self(self, it.fi, answer[u]);
            answer[u]+=answer[it.fi];
        }
        answer[u]+=dummy[u];
        return;
    };
