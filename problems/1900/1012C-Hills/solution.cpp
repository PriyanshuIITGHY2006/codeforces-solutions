...
void solve()
{
    ...
    int k = (n + 1) / 2;
    vector<vvi> dp(3, vvi(k + 1, vi(2, INF)));
    
    rep(i, 0, 3) dp[i][0][0] = 0;
    ...
    rep(i, 0, n)
    {
        ...
        rep(j, 1, mx + 1)
        {
            if (i >= 1) dp[c][j][0] = min(dp[p][j][0], dp[p][j][1]);

            int s = 0, l = 0;
            if (i >= 1) s += max(0ll, a[i - 1] - a[i] + 1);
            if (i < n - 1) s += max(0ll, a[i + 1] - a[i] + 1);
            if (i < n - 1) l += max(0ll, a[i + 1] - a[i] + 1);
            if (i >= 2) l += max(0ll, min(a[i - 1], a[i - 2] - 1) - a[i] + 1);
            ...
            int v1 = INF, v2 = INF;
            if (dp[pp][j - 1][0] != INF) v1 = s + dp[pp][j - 1][0];
            if (dp[pp][j - 1][1] != INF) v2 = l + dp[pp][j - 1][1];
    ...
    rep(i, 1, k + 1)
    {
        cout << min(dp[(n - 1) % 3][i][0], dp[(n - 1) % 3][i][1]) << (i == k ? "" : " ");
    }
    cout << "\n";
}
