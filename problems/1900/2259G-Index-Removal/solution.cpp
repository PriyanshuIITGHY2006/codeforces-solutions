...
#define YES cout<< "YES\n";
#define NO cout<< "NO\n";
int fin(const vector<int>& arr, int j, int target) {
    int low = 0;
    int high = j;
    int result = -1;
...
void solve() {
    ...
    rep(i, 1, n-1)
    {
        if(b[i]+b[i-1]<=k) ans.pb(0);
        else
         {int cost = (-k+(b[i]+b[i-1]));
            int indi = fin(d, i-2, cost + d[i-1]);
            
            int hi = cost;
            cost = hi * (i - 1 - indi); 
            if (i >= 2) { 
                int c_sub = c[i-2];
                if (indi >= 0) c_sub -= c[indi];
                cost -= c_sub;
                cost += (indi + 1) * (d[indi+1] - d[i-1]);
            }
            ans.pb(cost);
        }
    }
