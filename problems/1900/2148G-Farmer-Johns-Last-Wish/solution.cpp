...
void solve() {
    ...
    vi freq_cnt(n + 5, 0); 
    set<int, greater<int>> active_freqs;
    
    auto add_freq = [&](int f) {
        if (freq_cnt[f] == 0) active_freqs.insert(f);
        freq_cnt[f]++;
    };
    ...
    auto rem_freq = [&](int f) {
        if (freq_cnt[f] > 0) {
            freq_cnt[f]--;
            if (freq_cnt[f] == 0) active_freqs.erase(f);
        }
    };
    ...
    int cur = a[0];
    int hissi = a[0];
    const vi& dhinchak = pre_divs[a[0]];
    
    rep(j, 0, sz(dhinchak))
    {
        if(dhinchak[j] > hissi)
        {
            rem_freq(yo[dhinchak[j]]);
            add_freq(yo[dhinchak[j]]+1);
            yo[dhinchak[j]]++;
        }
        else yo[dhinchak[j]]++;
    }
    
    int zeros_count = sz(yo) - (a[0] + 1) + 1; 
    if (zeros_count > 0) {
        freq_cnt[0] += zeros_count;
        active_freqs.insert(0);
    }
    ...
    rep(i, 1, n)
    {
        const vi& dhinchak = pre_divs[a[i]];
        cur = __gcd(cur, a[i]);
        ...
        auto it = active_freqs.upper_bound(i + 1);
        cout << *it << " ";
    }
    nl;
}
