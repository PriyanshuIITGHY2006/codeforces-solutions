...
void solve() {
    ...
    rep(i, 0, m)
    {
        rep(j, max(maxi+1, a[i].fi), a[i].se+1){
            hello[j]=a[i].fi;
        }
    ...
    rep(i, 0, n+1)
    {
        if(hello[i]==0) hello[i]=i;
    }
    rep(i, 0, m)
    {
        freq[a[i].fi]++; freq[a[i].se+1]--;
    }
    rep(i, 1, n+2) freq[i]+=freq[i-1];
    vi dp(n+1, 0);
    rep(i, 1, n+1)
    {
        dp[i] = max(freq[i] + dp[hello[i]-1], dp[i-1]);
    }
    o(dp[n]);
}
