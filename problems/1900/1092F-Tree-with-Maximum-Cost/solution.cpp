...
void solve()
{
    ...
    nodeans[1] = cal1;
    dbg(cal1);
    auto reroute = [&](auto && self, int u, int p) -> void {
        if(p!=-1)
        {
            int k = subtree[p];
            int g = subtree[u];
            int nk = subtree[p]-subtree[u];
            int ng = subtree[u] + nk;
            int ans = nodeans[p] + nk - g;
            nodeans[u]=ans;
            subtree[u] = ng;
            subtree[p] = nk;
            dbg("START"); dbg(u); dbg(p);
            dbg(k);
            dbg(g);
            dbg(nk);
            dbg(ng);
