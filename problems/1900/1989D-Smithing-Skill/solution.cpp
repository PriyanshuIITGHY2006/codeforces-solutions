...
#define dbg_assert(c)                                                            \
    {                                                                            \
        if (!(c))                                                                \
        {                                                                        \
            cerr << DR "[ASSERT FAILED] " #c " line " << __LINE__ << DX << "\n"; \
            exit(1);                                                             \
        }                                                                        \
    }
#define dbg_time()                                                                                                  \
    {                                                                                                               \
        static auto _t = chrono::steady_clock::now();                                                               \
        auto _n = chrono::steady_clock::now();                                                                      \
        cerr << DR "[TIME] " << chrono::duration_cast<chrono::milliseconds>(_n - _t).count() << "ms" << DX << "\n"; \
        _t = _n;                                                                                                    \
    }
#else
...
#define YES cout << "YES\n";
#define NO cout << "NO\n";
ll dpc(int i, vll & dp, vpll & s)
{
    if(dp[i]==-1)
    {
        dp[i]=0;
        ll target = i;
        auto it = upper_bound(all(s), target, [](ll val, const pll &p)
                                  { return val < p.fi; });
            if (it != s.begin())
            {
                --it;
                ll max_val = it->fi;
                ll matching_se = it->se;
                ll time = ((target - max_val)/ matching_se)+1;
                target -= (time * max_val);
                target += (time * (max_val - matching_se));
                dp[i] = (time * 2) + dpc(target, dp, s);
            }
    }
    return dp[i];
}
void solve()
{
    ...
    srt(d);
    vll dp(1e6+1, -1); dp[0]=0;
    for (int i = 0; i < n; i++)
    {
        f[d[i].fi] = (d[i].fi - d[i].se);
    }
    vpll s;
    ll ans = INF;
    for (auto it : f)
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        while (1)
        {
            if(target>1e6)
            {
                ll max_val = s[sz(s)-1].fi;
                ll matching_se = s[sz(s)-1].se;
                ll time = ((target - max_val)/ matching_se)+1;
                target -= (time * max_val);
                target += (time * (max_val - matching_se));
                experience += (time * 2);
