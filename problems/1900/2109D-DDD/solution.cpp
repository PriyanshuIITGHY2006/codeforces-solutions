...
void solve() {
    ...
    vvll dist(n + 1, vi(2, INF));
    priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, greater<tuple<int, int, int>>> pq;
    dist[1][0] = 0;
    dist[1][1] = 0;
    pq.push(make_tuple(INF, 1, 1));
    pq.push(make_tuple(0, 0, 1));
    while (!pq.empty()) {
        auto [d, type, u] = pq.top(); pq.pop();
        if (d > dist[u][type]) continue;
        for (auto [v, w] : adj[u]) {
            if (dist[u][type] + w < dist[v][1-type]) {
                dist[v][1-type] = dist[u][type] + w;
                pq.push(make_tuple(dist[v][1-type], 1-type, v));
            }
        }
    }
    srt(a);
    ...
    even_sum = accumulate(all(even), 0ll);
    odd_sum = accumulate(all(odd), 0ll);
    int ac_odd = odd_sum, ac_even = even_sum;
    if(odd_sum%2==0)
    {
        ac_even+=odd_sum;
        if(odd_sum!=0)
        {
            ac_odd += (even_sum - odd[0]);
        }
    }
    else
    {
        ac_even += (odd_sum - odd[0]);
        ac_odd += even_sum;
    }
    string s = "1";
    rep(i, 2, n+1)
    {
        if(dist[i][0]<=ac_even || dist[i][1]<=ac_odd) s.pb('1');
        else s.pb('0');
    }
    o(s);
}
