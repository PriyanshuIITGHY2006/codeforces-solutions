...
void solve()
{
    ...
    int required_left = (sz(s) / 2) - left;
    int required_right = (sz(s) / 2) - right;
    if (required_left == 0 || required_right == 0)
    {
        YES return;
    }
    vi question, question1;
    ...
    for (int i = 0; i < sz(question); i++)
    {
        
            int next = question1[0];
            auto it = lower_bound(all(zeroes), question[i]);
            auto it1 = lower_bound(all(zeroes), next);
            if (it1 != zeroes.begin())
                it1--;
            if (it>it1)
            {
