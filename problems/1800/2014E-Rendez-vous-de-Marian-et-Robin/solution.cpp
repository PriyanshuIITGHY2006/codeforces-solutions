...
#define YES cout << "YES\n";
#define NO cout << "NO\n";
vvi sexy_djikstra(int &n, vector<vpii> &adj, vi &horses, int src)
{
    ...
    while (!pq.empty())
    {
        ...
        if (u.se == 0)
        {
            if (d > dist[u.fi][0])
                continue;
            for (auto [v, w] : adj[u.fi])
            {
                if (horses[u.fi])
                {
                    if (dist[u.fi][0] + (w / 2) < dist[v][1])
                    {
                        dist[v][1] = dist[u.fi][0] + (w / 2);
                        pq.push({dist[v][1], {v, 1}});
                    }
                }
                else
                {
                    if (dist[u.fi][0] + w < dist[v][0])
                    {
                        dist[v][0] = dist[u.fi][0] + w;
        ...
        else
        {
            ...
            for (auto [v, w] : adj[u.fi])
            {
                if (dist[u.fi][1] + (w / 2) < dist[v][1])
                {
                    dist[v][1] = dist[u.fi][1] + (w / 2);
                    pq.push({dist[v][1], {v, 1}});
                }
            }
        }
...
void solve()
{
    ...
    vvi dist1 = sexy_djikstra(n, adj, horses, 1);
    vvi distn = sexy_djikstra(n, adj, horses, n);
    int answer = INF;
    for (int i = 1; i <= n; i++)
    {
        answer = min(answer, max(min(dist1[i][0], dist1[i][1]) , min(distn[i][0], distn[i][1])));
    }
    if (answer == INF)
        o(-1);
    else
