...
void solve() {
    ...
    int best_c = -1;
    for (int c = 0; c < 26; ++c) {
        if (first_occ[c] != IINF && last_occ[c] != -1) {
            int len = first_occ[c] - last_occ[c] + m;
            if (len < min_len) {
                min_len = len;
                best_c = c;
            }
    ...
    if (best_c == -1) {
        ...
        string ans = s1.substr(0, first_occ[best_c]) + 
                     char(best_c + 'a') + 
                     s2.substr(last_occ[best_c] + 1);
        cout << ans << "\n";
    }
}
