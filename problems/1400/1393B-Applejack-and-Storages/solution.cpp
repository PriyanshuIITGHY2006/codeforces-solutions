...
void solve() {
    ...
    int count_4 = 0, count_2 = 0, count_6 = 0, count_8 = 0;
    map<int, int> m;
    rep(i, 0, n) m[a[i]]++;
    for(auto it : m)
    {
    ...
    while(q--)
    {
        ...
        dbgm(m);
        dbg3(count_2, count_4, count_6); dbg(count_8);
        if(count_8>0) {YES continue;}
        if(count_6>0 && count_2>count_6) {YES continue;}
        if(count_4>1){YES continue;}
        if(count_4>0 && count_2>count_4+1) {YES continue;}
        NO;
    }
}
