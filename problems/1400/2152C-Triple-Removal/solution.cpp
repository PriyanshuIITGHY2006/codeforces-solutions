...
#define NO cout<< "NO\n";
void solve() {
    int n, q; r(n, q); vi a = rvec(n);
    vi zeroes(n+1, 0), ones(n+1, 0);
    rep(i, 0, n)
    {
        zeroes[i+1] = zeroes[i];
        ones[i+1] = ones[i];
    ...
    {
        int li, ri; r(li, ri);
        if((zeroes[ri]-zeroes[li-1])%3!=0 || (ones[ri]-ones[li-1])%3!=0)
        {
            o(-1);
        }
        else
        {
            int answer = ((zeroes[ri]-zeroes[li-1]) + (ones[ri]-ones[li-1]))/3 + 1;
            auto it = lower_bound(all(hejbollah), li);
            if(it!=hejbollah.end())
            {
                if(*it <= ri-1) answer--;
            }
