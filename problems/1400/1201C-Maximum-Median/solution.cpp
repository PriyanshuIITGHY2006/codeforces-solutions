...
void solve() {
    ...
    int numbers=0;
    ll answer=freq[0].first;
    for(int i=0; i<freq.size()-1; i++)
    {
        numbers +=freq[i].second;
        ll t = k/numbers;
        ll increase = min(t, freq[i+1].first-freq[i].first);
        answer+=increase;
        k -= (increase*numbers);
        if(i==freq.size()-2) numbers+=freq[freq.size()-1].second;
        if(t<freq[i+1].first-freq[i].first)
        {
            o(answer);
            return;
        }
    }
    if(numbers==0) numbers=freq[0].second;
    ll t =k/numbers;
    answer +=t;
    o(answer);
