...
void solve()
{
    ...
    temp1.pb(0);
    temp2.pb(0);
    while (first + i < 2*n && first - i >= 0)
    {
        if (a[first + i] == a[first - i])
        {
            temp1.pb(a[first + i]);
            temp1.pb(a[first + i]);
        }
    ...
    i = 1;
    while (second + i < 2*n && second - i >= 0)
    {
        if (a[second + i] == a[second - i])
        {
            temp2.pb(a[second + i]);
            temp2.pb(a[second + i]);
        }
    ...
    bool possible = true;
    int left = first + 1, right = second - 1;
    while (left <= right)
    {
        if (a[left] != a[right])
        {
    ...
    int tepi1 = find_mex(temp1);
    int tepi2 = find_mex(temp2);
    int answer = max(tepi1, tepi2);
    if (possible)
    {
        rep(j, first, second + 1) temp3.pb(a[j]);
        i = 1;
        while (second + i < 2*n && first - i >= 0)
        {
            if (a[second + i] == a[first - i])
            {
                temp3.pb(a[first - i]);
                temp3.pb(a[first - i]);
            }
        ...
        answer = max(answer, find_mex(temp3));
    }
    o(answer);
}
