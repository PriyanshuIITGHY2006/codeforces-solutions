...
void solve()
{
    ...
    per(i, 0, g)
    {
        ...
        {
            temp.erase(temp.find(c[i]));
            rep(j, 0, i)
            {
                if (temp.find(c[j]) != temp.end())
                {
                    temp.erase(temp.find(c[j]));
                }
            }
            if (sz(temp) == 0)
            {
        ...
        else if (temp1.find(c[i]) != temp1.end())
        {
            temp1.erase(temp1.find(c[i]) != temp1.end());
            rep(j, 0, i)
            {
                if (temp.find(c[j]) != temp.end())
                {
