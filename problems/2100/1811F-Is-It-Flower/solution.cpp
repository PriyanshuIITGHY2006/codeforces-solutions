...
void solve()
{
    ...
    if (!(count4 == g && count2 == n - g))
    {
        NO return;
    }
    vector<bool> somebody_get_these_beggeRs_out_of_here(n + 1, true);
    for (auto it : temp)
    {
        // o(it);
        int start = -1, end = -1, huehue = 0;
        for (auto it1 : adj[it])
        {
            if (degrees[it1] == 2)
            {
        ...
        if (start == -1 || end == -1 || huehue != 2)
        {
            NO return;
        }
        int count = 1;
        while ((start != end) && count != g - 1)
        {
            // o(start);
            somebody_get_these_beggeRs_out_of_here[start] = false;
            int ribba = 0;
            for (auto j : adj[start])
            {
                if (degrees[j] == 2 && somebody_get_these_beggeRs_out_of_here[j])
                {
                    start = j;
                    ribba++;
                }
    ...
    int start = temp[0];
    int counti = 0;
    while (somebody_get_these_beggeRs_out_of_here[start])
    {
        somebody_get_these_beggeRs_out_of_here[start] = false;
        bool ok = false;
        counti ++;
        for (auto j : adj[start])
        {
            if (degrees[j] == 4 && somebody_get_these_beggeRs_out_of_here[j])
            {
                start = j;
                ok = true;
            }
