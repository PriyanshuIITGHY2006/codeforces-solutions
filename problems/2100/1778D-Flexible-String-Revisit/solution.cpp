...
void solve()
{
    ...
    per(i, 1, n)
    {
        int a = (modinv(n) * (i)) % MOD;
        int b = (modinv(n) * (n-i)) % MOD;
        dp[i][0] = (1 + (b * dp[i + 1][0]) % MOD) % MOD;
        dp[i][1] = a;
        int denom = (1 - ((b * dp[i + 1][1]) % MOD) + MOD) % MOD;
        dp[i][0] = (dp[i][0] * modinv(denom)) % MOD;
        dp[i][1] = (dp[i][1] * modinv(denom)) % MOD;
    }
    vi ans(n + 1);
    rep(i, 1, n + 1)
    {
        ans[i] = (dp[i][0] + ((ans[i - 1] * dp[i][1]) % MOD)) % MOD;
    }
    int g=0;
    rep(i, 0, n)
    {
