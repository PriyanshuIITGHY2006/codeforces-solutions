...
void solve()
{
    ...
    rep(i, 0, n) if (a[i] != -1) s.erase(a[i]);
    vi minuses;
    for (auto it : s)
        minuses.pb(it);
    vi b(n);
    ...
    {
        int first = sz(s);
        int second = ((sz(s) * (sz(s) - 1)) / 2) % MOD;
        rep(i, 0, n)
        {
            if (a[i] == -1)
            {
                int g = b[i] - 1;
                inv = (inv + ((((((g * modinv(first)) % MOD) * second) % MOD) * modinv(first - 1)) % MOD)) % MOD;
            }
            else
            {
                auto it = upper_bound(all(minuses), a[i]);
                int k = minuses.end() - it;
                inv = (inv + ((((b[i] * modinv(first)) % MOD) * k) % MOD)) % MOD;
                int ki = first - b[i];
                inv = (inv + ((((ki * modinv(first)) % MOD) * (first - k)) % MOD)) % MOD;
            }
        }
    }
    ordered_set hailon;
    rep(i, 0, n)
    {
        if (a[i] != -1)
        {
            inv = (inv + sz(hailon) - hailon.order_of_key(a[i])) % MOD;
            hailon.insert(a[i]);
        }
    }
    o(inv);
