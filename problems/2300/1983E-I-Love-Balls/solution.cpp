...
void solve() {
    int n, k; r(n, k); vi a = rvec(k); vi b = rvec(n-k);
    int odd = 0, even = 0;
    int hello = accumulate(all(b), 0ll)%MOD;
    int hi = accumulate(all(a), 0ll)%MOD;
    if(n-k==0){
        o(accumulate(all(a),0ll)%MOD, 0); return;
    }
    if((n-k)%2!=0){
        // hello = (hello * modinv(n-k))%MOD;
        odd = (((((n-k+1)/2)*modinv(n-k))%MOD)*hello)%MOD;
        even = (((((n-k)/2)*modinv(n-k))%MOD)*hello)%MOD;
        int g = n-k+1;
        odd = (odd + (hi*modinv(2))%MOD)%MOD;
        even = (even + (hi*modinv(2))%MOD)%MOD;
        o(odd, even);
    }
    else{
        odd = (hello*modinv(2))%MOD;
        even = odd;
        int g = n-k+1;
        int odd1 = (((((n-k+2)/2)*modinv(n-k+1))%MOD)*hi)%MOD;
        int even1 = (((((n-k)/2)*modinv(n-k+1))%MOD)*hi)%MOD;
        odd = (odd+odd1)%MOD;
        even = (even + even1)%MOD;
        o(odd, even);
    }
