...
ordered_multiset;
void solve() {
    int n; r(n); vi a = rvec(n); vi b = rvec(n);
    vi vals;
    rep(i, 0, n) {
        vals.pb(a[i]);
    ...
    Fenwick hikka(sz(vals));
    Fenwick hulle(sz(vals));
    int sz_hikka = 0;
    int sz_hulle = 0;
    ...
    rep(i, 0, n)
    {
        rep(j, 0, n)
        {
            int g = hikka.query(get_id(a[i] * b[j]));
            int hii = sz_hikka - g;
            
            int h = hulle.query(get_id(a[i]));
            int hiiiuu = sz_hulle - h;
        ...
        rep(j, 0, n)
        {
            hikka.add(get_id(a[i] * b[j]), 1);
            sz_hikka++; 
        }
        hulle.add(get_id(a[i]), 1);
        sz_hulle++;
    }
    o((total * modinv(fact[n])) % MOD);
}
