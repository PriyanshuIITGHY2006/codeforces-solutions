...
#define YES cout<< "YES\n";
#define NO cout<< "NO\n";
vvll multiply(const vvll & mat1, const vvll & mat2, int g)
{
    vvll rslt(g, vll(g));
    ...
    for (int i = 0; i < g; i++) {
        for (int j = 0; j < g; j++) {
            ...
            for (int k = 0; k < g; k++) {
                rslt[i][j] = (rslt[i][j] + (mat1[i][k] * mat2[k][j])%MOD)%MOD;
            }
        }
    }
...
power(vector<vector<long long> > M, ll expo, int g)
{
    // Initialize result with identity matrix
    vector<vector<long long> > ans(g, vll(g,0));
    for(int i=0; i<g; i++)
    {
        ans[i][i]=1;
    }
...
void solve() {
    ll a, b, k, n; r(a, b, k, n);
    vvll trs (k, vll(k, 0));
    ll p = ((a%MOD)* modinv(b))%MOD;
    ll q = (1- p + MOD)%MOD;
    for(int i=0; i<k; i++)
    {
        trs[0][i]=q;
    ...
    vvll res = power(trs, n, k);
    ll result=0;
    for(int i=0; i<k; i++)
    {
        result = (result + res[i][0])%MOD;
    }
    o(result);
}
