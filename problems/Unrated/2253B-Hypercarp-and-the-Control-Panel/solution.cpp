...
void solve()
{
    ...
    rep(i, 0, sz(hello)-1)
    {
        if(hello[i]>1 && hello[i+1]>1){
            o(answer+2); return;
        }
    }
    if(sz(hello)>=2)
    {
        if(hello[0]==1 && hello[1]>1) {
            o(answer+1); return;
        }
        if(hello.back()==1 && hello[sz(hello)-2]>1) {
            o(answer+1); return;
        }
        rep(i, 0, sz(hello)-2)
        {
            if(hello[i]>1 && hello[i+1]==1 && numbers[i+2]!=numbers[i])
            {
                o(answer+1); return;
            }
        }
