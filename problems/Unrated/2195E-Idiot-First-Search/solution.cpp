    ...
    auto dfs = [&](auto &&self, int parent, int node)->void{
        ...
        for(auto it: adj[node])
        {
            if(it!= parent) {
                visited=true;
                self(self, node, it);
                answer = (answer%MOD + number[it]%MOD)%MOD;
            }
        }
        if(visited)number[node] = (answer + 4)%MOD;
    };
    dfs(dfs, -1, 0);
    dbgv(number);
    vll act_answer(n+1,0);
    number[0]=0;
    // vi height(n+1,0);
    auto dfs1 = [&](auto &&self, int parent, int node)->void{
        if(parent!=-1){ 
            // height[node]=height[parent]+1;
            act_answer[node]=(act_answer[parent]%MOD+number[node]%MOD+1)%MOD;
        }
        for(auto it: adj[node])
        {
            if(it!= parent) {
