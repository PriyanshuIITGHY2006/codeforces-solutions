...
void solve()
{
    ...
    {
        auto it = st.find(s[i] - '0');
        auto it1 = st.lower_bound(s[i] - '0');
        auto it2 = st.upper_bound(s[i] - '0');
        auto it3 = st.end();
        it3--;
        auto it4 = st.begin();
        if (it != st.end())
        {
            ...
            if (it1 != st.begin())
            {
                it1--;
                temp1.pb(*it1 + '0');
                temp1.append(sz(s) - i - 1, *it3 + '0');
                ll tempi1 = stoll(temp1);
                ans = min(ans, abs(tempi1 - number));
            }
            if (it2 != st.end())
            {
                temp2.pb(*it2 + '0');
                temp2.append(sz(s) - i - 1, *it4 + '0');
                ll tempi2 = stoll(temp2);
                ans = min(ans, abs(tempi2 - number));
            }
            answer.pb(s[i]);
        }
        else
    ...
    if (answer != "")
        ans = min(ans, abs(stoll(answer) - number));
    string check1 = "";
    string check2 = "";
    auto it1 = st.begin();
    auto it2 = st.end();
    it2--;
    check1.append(sz(s) - 1, *it2 + '0');
    auto it_nonzero = st.upper_bound(0);

    if (it_nonzero != st.end())
    {
        check2.pb(*it_nonzero + '0'); 
        check2.append(sz(s), *st.begin() + '0');
    }
    if (check1 != "")
        ans = min(ans, abs(-stoll(check1) + number));
    if (check2 != "") ans = min(ans, abs(stoll(check2) - number));
    o(ans);
}
