...
void solve()
{
    ...
    while (t <= n)
    {
        ...
        {
            hulle.insert(t - 2);
            hulle.insert(a[t]); // <--- FIX: Only insert the base prime (like 3 or 5). Blocks ONLY the first gap!
            ...
            while (g <= n)
            {
                if (g % 2 != 0 && !hikka[g])
                {
                    ans.pb(g);
                    hikka[g] = true;
                    // REMOVED the line that was adding every 'g' to hulle
                }
                g += a[t];
            }
            if (!hikka[t])
    ...
    rep(i, 1, sz(ans))
    {
        if (hulle.find(ans[i]) == hulle.end() && hulle.find(ans[i - 1]) == hulle.end())
        {
            if (cur < sz(left_out))
            {
                ans1.pb(left_out[cur]);
                cur++;
                ans1.pb(ans[i]);
                if (i + 1 < sz(ans))
                {
