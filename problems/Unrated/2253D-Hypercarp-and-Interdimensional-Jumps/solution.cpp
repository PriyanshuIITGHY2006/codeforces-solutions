...
void solve()
{
    ...
    int t = (steps * (steps + 1)) / 2;
    int hikka = (x + t - y) / 2;
    {
        int c1 = hikka, c2 = hikka + 1;
        auto dist = [&](int cand) -> int {
            int xi, yi;
            if (cand <= 0) { xi = 0; yi = t; }
            else if (cand >= t) { xi = t; yi = 0; }
            else { xi = cand; yi = t - cand; }
            if (xi > x || yi > y) return INF;
            return (x - xi) * (x - xi) + (y - yi) * (y - yi);
        };
        if (dist(c2) < dist(c1)) hikka = c2;
    }
    int xi, yi;
    if (hikka <= 0)
    {
    ...
    int g = hikka - ((req * (req + 1)) / 2);
    int removed = req + 1 - g;
    string ans = "";
    rep(i, 1, steps + 1)
        ans.pb((i <= req + 1 && i != removed) ? 'X' : 'Y');
    rev(ans); o(ans);
}
