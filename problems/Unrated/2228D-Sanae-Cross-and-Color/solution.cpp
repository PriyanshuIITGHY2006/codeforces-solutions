...
void solve()
{
    ...
    srt(a);
    int answer = 0;
    vi gh(n), ij(n), kl(n), mo(n);
    int cmin = -INF, cmax = INF;
    rep(i, 0, n)
    {
        gh[i] = min(cmax, a[i].se);
        ij[i] = max(cmin, a[i].se);
        cmax = gh[i];
        cmin = ij[i];
    }
    cmax = INF, cmin = -INF;
    per(i, 0, n)
    {
        kl[i] = min(cmax, a[i].se);
        mo[i] = max(cmin, a[i].se);
        cmax = kl[i];
        cmin = mo[i];
    }
    vi xcord(n + 1, 0), ycord(n + 1, 0);
    rep(i, 0, n)
    {
        ycord[a[i].se]=1;
        xcord[a[i].fi]=1;
    ...
    }
    {
        rep(i, 0, sz(temp))
        {
            int a = max(gh[temp[i]], kl[temp[i]+1]), b = min(ij[temp[i]], mo[temp[i]+1]);
            if (a>=b)
            {
                continue;
            }
