...
#define NO cout<< "NO\n";
void solve() {
    int n, a, b; r(n, a, b); vi will_this_work = rvec(n);
    int hi_lol = __gcd(a, b);
    // int maxi = -INF, mini = INF;
    rep(i, 0, n)
    {
        will_this_work[i]%=hi_lol;
        if(will_this_work[i]==0) will_this_work[i]+=hi_lol;
    }
    uni(will_this_work);
    int hulle = mx(will_this_work)-mn(will_this_work);
    rep(i, 1, sz(will_this_work))
    {
        hulle = min(will_this_work[i-1]+hi_lol - will_this_work[i], hulle);
    }
    o(hulle);
}
