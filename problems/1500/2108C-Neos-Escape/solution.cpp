...
void solve()
{
    ...
    rsrt(c);
    set<pair<int, int>> hallabol;
    set<pair<int, int>, SortBySecond> hallabol1;
    rep(i, 0, sz(c))
    {
        auto it = hallabol.lower_bound(mp(c[i].se.se + 1, -INF));
        auto it1 = hallabol1.lower_bound(mp(-INF, c[i].se.fi - 1));
        if (it != hallabol.end() && it->fi == c[i].se.se + 1)
        {
            int li = it->fi, chi = it->se;
            hallabol.erase(mp(li, chi));
            hallabol1.erase(mp(li, chi));
            hallabol.insert({c[i].se.fi, chi});
            hallabol1.insert({c[i].se.fi, chi});
        }
        else if (it1 != hallabol1.end() && it1->se == c[i].se.fi - 1)
        {
            int li = it1->fi, chi = it1->se;
            hallabol.erase(mp(li, chi));
            hallabol1.erase(mp(li, chi));
            hallabol.insert({li, c[i].se.se});
            hallabol1.insert({li, c[i].se.se});
        }
        else
        {
            hallabol.insert(mp(c[i].se.fi, c[i].se.se));
            hallabol1.insert(mp(c[i].se.fi, c[i].se.se));
        }
    }
    o(sz(hallabol1));
}
