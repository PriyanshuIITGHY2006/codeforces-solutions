...
for(auto it :m)
{
    if(it.second/2>0)mini=min(mini,it.first);
    possible_pairs+=(it.second/2);
    int temp=(it.first*(it.second/2)*2);
    permimeter+=temp;
...
if(possible_pairs>=1)
{
    ...
    for(auto it :m)
{
    if(it.second%2==1) maxi=max(maxi, it.first);
}
int sec_maxi=-1;
 for(auto it :m)
{
    dbg(it.second);
    if(it.second==1 && it.first!=maxi) sec_maxi=max(sec_maxi, it.first);
}
dbg(maxi);
dbg(mini);
dbg(sec_maxi);
if(maxi!=-1) {if(maxi<permimeter) permimeter+=maxi;
    else if(possible_pairs==1) permimeter=0;
if((possible_pairs*2+1)%2==1)
{
    if(sec_maxi!=-1 && sec_maxi<permimeter) permimeter+=sec_maxi;
