...
void solve()
{
    ...
    while (queries--)
    {
        ...
        r(l, u);
        int obj = pre[l - 1] + u;
        auto it1 = lower_bound(all(pre), obj);
        auto it2 = lower_bound(all(pre), obj);
        it2--;
        if (it1 != pre.end())
        {
            int h1 = *it1 - pre[l - 1];
            int gain = (h1 * u) - ((h1 * (h1 - 1)) / 2);
            int ri = it1 - pre.begin();
            if (*it2 >= pre[l])
            {
                int h2 = *it2 - pre[l - 1];
                int gain2 = (h2 * u) - ((h2 * (h2 - 1)) / 2);
                if (gain2 >= gain)
                {
                    int hikka = it2 - pre.begin();
                    ri = min(ri, hikka);
                }
            }
            cout << ri << " ";
