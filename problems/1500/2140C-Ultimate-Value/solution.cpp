...
void solve() {
    ...
    priority_queue <ll> max1, max2;
    ll answer1=0;
    ll answer2=0;
    for(int i=1; i<oddr.size(); i++)
    {
        max1.push(evenl[i-1]);
        answer1=max(answer1, oddr[i]+max1.top());
    }
    for(int i=0; i<evenr.size(); i++)
    {
        max2.push(oddl[i]);
        answer2=max(answer2, evenr[i]+max2.top());
    }
    ll answer3=0;
    if(n%2==0) answer3=n-2;
    else answer3=n-1;
    ll answer= max(answer3, max(answer1, answer2));
    cout<<sum+answer<<"\n";
}
