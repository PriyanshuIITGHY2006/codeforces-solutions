...
void solve()
{
    ...
    while (m--)
    {
        ...
        int minimum = lcm(a, b);
        int first = lcm(a, b) / a;
        int second = lcm(a, b) / b;
        int plusi = plus;
        int minusi = minus;
        if (plusi > minusi)
            swap(plusi, minusi);
        int high = first + second;
        int first_diff = minusi - plusi;
        int second_diff = -first + second;
        if (second_diff != 0)
        {
            int hi = (first_diff / second_diff);
            if((minusi - second * hi) >=0 && (plusi - first * hi) >=0)
            {if (minusi - second * hi == plusi - first * hi)
                {YES continue; } else {NO continue;}}
        }
        {
            if (first_diff == 0)
                YES else NO
