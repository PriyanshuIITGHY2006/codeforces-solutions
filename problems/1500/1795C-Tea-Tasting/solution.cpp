...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        auto it = upper_bound(all(pre), b[i] + pre[i]);
        if (it != pre.begin())
            it--;
        ...
        if (1 + index < n + 1)
            taster[1 + index] -= 1;
        int remaining = b[i] + pre[i] - *it;
        if (index + 1 < n + 1)
            taster1[index + 1]  +=remaining;
    }
    vll pre1(n + 1, 0);
    for (int i = 0; i < n; i++) pre1[i+1] = pre1[i] + taster[i+1];
    for(int i=1; i<n+1; i++)
    {
      cout<< pre1[i]*a[i-1] + taster1[i] << " ";
    }
    nl;
}
