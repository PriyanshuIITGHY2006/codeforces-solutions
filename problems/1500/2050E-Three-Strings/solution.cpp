...
#define YES cout<< "YES\n";
#define NO cout<< "NO\n";
ll dpc(vvll&dp, ll i, ll j, string&a, string&b, string&c, int &m, int&n)
{
    if(dp[i][j]==-1)
    {
    if(i==m && j==n)
    ...
    {
        if(c[i+j]!=b[j]) dp[i][j]= 1+dpc(dp, i, j+1, a, b, c, m, n);
        else {
            dp[i][j]=dpc(dp, i, j+1, a, b, c, m, n);
        }
    }
    else if (j==n)
    {
        if(c[i+j]!=a[i]) dp[i][j]= 1+dpc(dp, i+1, j, a, b, c, m, n);
        else {
            dp[i][j]=dpc(dp, i+1, j, a, b, c, m, n);
        }
    }
    else
    {if(c[i+j]!=a[i] && c[i+j]!=b[j])
    {
        dp[i][j] = 1 + min(dpc(dp, i+1, j, a, b, c, m, n), dpc(dp, i, j+1, a, b, c, m, n));
    }
    else if (a[i]==b[j])
    {
        dp[i][j]=min(dpc(dp, i+1, j, a, b, c, m, n), dpc(dp, i, j+1, a, b, c, m, n));
    }
    else if(c[i+j]==a[i])
    {
        dp[i][j]=min(1+dpc(dp, i, j+1, a, b, c, m, n), dpc(dp, i+1, j, a, b, c, m, n));
    }
    else
    {
        dp[i][j]=min(dpc(dp, i, j+1, a, b, c, m, n), 1+dpc(dp, i+1, j, a, b, c, m, n));
    }}}
    return dp[i][j];
}
void solve() {
    ...
    int m = s1.length();
    int n = s2.length();
    vvll dp(m+1, vll(n+1,-1));
    o(dpc(dp, 0, 0, s1, s2, s3, m, n));
}
