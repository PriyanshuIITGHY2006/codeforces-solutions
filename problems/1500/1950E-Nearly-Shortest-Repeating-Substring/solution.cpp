...
void solve() {
    int n; r(n);string s; r(s);
    vi a = get_divisors(n);
    // pv(a);
    for(auto it : a){
        int g = n/it;
        vector<set<int>> hulle(it);
        int possible = true;
        rep(i, 0, it){
            int volcano = 1, volcanopi = 0;
            hulle[i].insert(s[i]-'a');
            rep(j, 1, g)
            {
                hulle[i].insert(s[i + (j*it)]-'a');
                if(s[i+(j*it)]!=s[i]) volcanopi++;
                else if(s[i+(j*it)]==s[i]) volcano++;
            }
            if(volcano>1 && volcanopi>1) possible = false;
        }
        if(!possible) continue;
        int count = 0;
        rep(i, 0, it) count+=sz(hulle[i]);
        if(count-it==0 || count - it == 1) {o(it); return;}
    }
}
