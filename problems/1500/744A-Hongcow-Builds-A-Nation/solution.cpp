...
#define NO cout<< "NO\n";
void solve() {
    int n, m, k; r(n,m,k); DSU a(n);
    vector<bool> gov(n+1, false);
    for(int i=0; i<k; i++)
    {
        int g; r(g); gov[g]=true;
    }
    for(int i=0; i<m; i++)
    {
        int li, ri; r(li, ri); a.unite(li, ri);
    }
    for(int i=1; i<n+1; i++) a.find(i);

    map<int, vi> freq;
    for(int i=1; i<n+1; i++) freq[a.parent[i]].pb(i);
    vll clusters;
    // ll max=-1;
    int total=0;
    for(auto it:freq)
    {
        ...
        for(auto it1 : it.se)
        {
            ...
            if(govi) clusters.pb(sz(it.se));
            else total += sz(it.se);
    }
    srt(clusters);
    // dbg(sz(clusters));
    clusters[sz(clusters)-1] +=(total);
    ll answer = 0;
