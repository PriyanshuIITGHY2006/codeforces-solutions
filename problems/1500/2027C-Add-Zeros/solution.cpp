...
#define YES cout << "YES\n";
#define NO cout << "NO\n";
int total(map<int, vi> &m, int curr, map<int, int> &dp)
{
    // int curi = 0;
    int maxi = -INF;
    // m[curr];
    ...
    else
    {
        for (int i = 0; i < sz(m[curr]); i++)
        {
            ...
            maxi = max(maxi, m[curr][i] - 1 + total(m, curr + m[curr][i] - 1, dp));
        }
        dp[curr] = maxi;
    }
    return dp[curr];
...
void solve()
{
    ...
    dbgm(m);
    int cur_state = 0;
    int answer = n + total(m, 0, dp);  o(answer);
}
