...
void solve() {
    ll m, sal; r(m, sal);
    vll happiness(m), values(m);
    for(int i=0; i<m; i++)
    {
        r(values[i], happiness[i]);
    }
    vll dp(5e4+1, INF);
    dp[0]=0;
    vector<bool> dp1(5e4+1, false);
    dp1[0]=true;
    for(int i=0; i<m; i++)
    {
        vpll container;
        ll max_money = i*sal;
        if(values[i]<=max_money)
        {
            for(int j=0; j<5e4+1; j++)
            {
                if(dp1[j]==true)
                {
                    if(dp[j]+values[i]<=max_money)
                    {
                        container.pb({j+happiness[i], dp[j]+values[i]});
                    }
                }
            }
