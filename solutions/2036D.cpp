...
void solve() {
    ...
    int lengthl=0, lengthr=m-1;
    int upperl=0, upperd=n-1;
    int count=0;
    while(lengthl<lengthr && upperl<upperd)
    {
        int i=lengthl, j = upperl;
        string temp="";
        for(int x=lengthl; x<lengthr; x++)
        {
            temp.pb(a[upperl][x]);
            if(sz(temp)==4) {
                if(temp=="1543") count++;
