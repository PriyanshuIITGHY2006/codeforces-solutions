...
void solve() {
    ...
    vi a = rvec(n);
    vvi dp(n+1, vi(n));
    //dp[i][j] = max(dp[i-1][j-1] + a[i] + x, dp[i][j-1] + a[i])
    // dp[0][0] = max(a[0], 0);
    vi answer(n+1,0);
    for(int i=0; i<n; i++)
    {
        if(i!=0) dp[0][i] = max(dp[0][i-1] + a[i], a[i]);
        else dp[0][0] = max(a[i], 0);
        for(int j=1; j<i; j++)
        {
            dp[j][i] = max(dp[j-1][i-1] + a[i] + x, dp[j][i-1] + a[i]);
        }
        if(i!=0)
        dp[i+1][i] = dp[i][i-1] + a[i] + x;
        else dp[1][0] = a[i] + x;
    }
    for(int i=0; i<n+1; i++)
    {
    ...
    for(int i=0; i<n+1; i++)
    {
        for(int j=((i==0) ? i : i-1); j<n; j++)
        {
            answer[i] = max(answer[i], dp[i][j]);
        }
    }
    for(int i=0; i<n+1; i++)
    {
