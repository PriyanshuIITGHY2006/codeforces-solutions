...
void solve()
{
    ...
    vi t = rvec(n);
    vpii adj;
    rep(i, 0, n) adj.pb({a[i], t[i]});
    srt(adj);
    vi maxi(n, -INF);
    maxi[n - 1] = adj[n - 1].fi + adj[n - 1].se;
    per(i, 0, n - 1)
    {
        maxi[i] = max(maxi[i + 1], adj[i].fi + adj[i].se);
    }
    ld cur_val = maxi[0];
    ld cur_pos = adj[0].fi;
    int mini = INF;
    rep(i, 0, n)
    {
        if (i != n - 1)
        {
            mini = min(mini, adj[i].fi - adj[i].se);
            ld dummy = (ld)(maxi[i + 1] + mini) / 2;
            if (dummy >= adj[i].fi && dummy <= adj[i + 1].fi)
            {
                if (max((ld)(maxi[i + 1] - dummy), (ld)(dummy - mini)) <= cur_val)
                {
                    cur_val = max((ld)(maxi[i + 1] - dummy), (ld)(dummy - mini));
                    cur_pos = dummy;
                }
            }
        }
        ld dummy = adj[i].fi;
        if (max((ld)(maxi[i] - dummy), (ld)(dummy - mini)) <= cur_val)
        {
            cur_val = max((ld)(maxi[i] - dummy), (ld)(dummy - mini));
            cur_pos = dummy;
        }
    }
