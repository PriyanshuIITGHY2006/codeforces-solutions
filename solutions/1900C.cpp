...
#define YES cout<< "YES\n";
#define NO cout<< "NO\n";
ll dfs( map<int, pair<int, pair<int, int>>> &m, string &s, int i, vll &dp)
{
    if(dp[i]==-1)
    {
        if(m[i].second.first==-1 && m[i].second.second==-1) dp[i]=0;
        else
        {
            ll first=INF; ll second=INF;
            if(m[i].second.first!=-1) first = dfs(m, s, m[i].second.first, dp);
            if(m[i].second.second!=-1) second = dfs(m, s, m[i].second.second, dp);

            if(s[i]=='L' ) dp[i] = min(first, 1+second);
            else if(s[i]=='R') dp[i] = min(second, 1+first);
            else dp[i]= 1+min(second, first);
        }
    }
    return dp[i];
...
void solve() {
    ...
    for(int i=0; i<n; i++)
    {
        ...
        {
            left--;
            m[i].second.first=left;
            m[left].first=i;
        }
        else m[i].second.first=-1;
        if(right!=0)
        {
            right--;
            m[i].second.second=right;
            // right--;
            m[right].first=i;
        }
        else
