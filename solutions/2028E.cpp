...
void solve() {
    ...
    auto dfs = [&](auto && self,int u, int parent) -> void{
        ...
        for(auto it: adj[u])
        {
            if(it!=parent)
            {
                self(self, it, u);
                leaves=false;
                heighti=min(heighti, height[it]);
            }
        }
        if(!leaves) height[u] = heighti+1;
    };
    dfs(dfs,1, -1);
    vll pcoeff(n+1,0);
    for(int i=1; i<n+1; i++)
    {
        ll temp = (1ll - (modinv(2)*pcoeff[i-1])%MOD + MOD) % MOD;
        pcoeff[i] = (modinv(2)*modinv(temp))%MOD;
    }
    vll answer(n+1,0);
    answer[1]=1;
    auto dfs1 = [&](auto && self, int u, int parent)->void{
        if(parent!=-1)
        {
            answer[u] = (answer[parent]*pcoeff[height[u]])%MOD;
        }
        for(auto it: adj[u])
        {
            if(it!=parent) self(self, it, u);
        }
    };
    dfs1(dfs1, 1, -1);
    for(int i=1; i<n+1; i++)
