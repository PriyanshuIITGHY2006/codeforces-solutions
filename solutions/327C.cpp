...
void solve() {
    ...
    ll k;
    cin>>k;
    ll key = ((binpow(binpow(2, k), s.length()) - 1)*modinv(binpow(2, s.length())-1))%MOD;
    vll power;
    int counter=0;
    for(auto it:s)
    {
        if((it-'0')%5==0) power.pb(counter);
        counter++;
    }
    ll sum_b_ans=0;
    for(auto it: power)
    {
        sum_b_ans=(sum_b_ans%MOD+binpow(2, it))%MOD;
    }
    ll answer =( sum_b_ans * key)%MOD;
    cout<<answer;
}
