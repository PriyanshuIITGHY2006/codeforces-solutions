...
void solve() {
    int n, q; r(n, q);
    vector<vi> adj(n + 1);
    rep(i, 0, n-1){ int u, v; r(u, v); adj[u].pb(v); adj[v].pb(u); }
    using F = pll;
    auto nrm = [&](F a)->F{ ll g = __gcd(a.fi < 0 ? -a.fi : a.fi, a.se); if(g > 1){ a.fi /= g; a.se /= g; } return a; };
    auto add = [&](F a, F b)->F{ return nrm({a.fi*b.se + b.fi*a.se, a.se*b.se}); };
    auto lt  = [&](F a, F b){ return (__int128)a.fi*b.se < (__int128)b.fi*a.se; };
    auto fmin = [&](F a, F b){ return lt(a, b) ? a : b; };
    F BIG = {INF, 1}, ZERO = {0, 1}, ONE = {1, 1};
    vector<vector<array<F,2>>> dp(n+1, vector<array<F,2>>(n+1, {BIG, BIG}));
    rep(i, 0, n+1) dp[1][i][0] = dp[1][i][1] = ZERO;
    auto dfs = [&](auto &&self, int u, int p)->void{
        if(u != 1)
        rep(i, 0, n+1){
            dp[u][i][1] = add(ONE, dp[p][i][0]);
            dp[u][i][0] = add(dp[p][i][1], F{2*sz(adj[u]) - 1, 1});
            if(i >= 1) dp[u][i][0] = fmin(dp[u][i][0], add(ONE, dp[p][i-1][1]));
        }
        for(auto it : adj[u]){
            if(it == p) continue;
            self(self, it, u);
    ...
    while(q--){
        int vik, p; r(vik, p);
        F f = dp[vik][p][1];
        o(f.fi % MOD * modinv(f.se % MOD) % MOD);
    }
}
