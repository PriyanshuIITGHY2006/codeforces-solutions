...
void solve()
{
    ...
    rep(i, 1, n)
    {
        ...
        {
            bad = (bad * fact[cur]) % MOD;
            cur = 1;
        }
    }
    bad = (bad * fact[cur]) % MOD;
    bad_permut = (bad + bad_permut)%MOD;
    bad = 1;
    cur = 1;
    rep(i, 1, n)
    {
    ...
    if (g == last)
    {
        ...
        rep(i, 0, n - 1)
        {
            if ((fast[i] != fast[i + 1]) || (last[i] != last[i + 1]))
            {
                temp.pb(i + 1);
            }
        }
        ...
        rep(i, 1, sz(temp))
        {
            chunaid_khan = (chunaid_khan * (fact[temp[i] - temp[i - 1]])) % MOD;
        }
        bad_permut = (bad_permut - chunaid_khan + MOD) % MOD;
    }
    o((fact[n] - bad_permut + MOD) % MOD);
}
