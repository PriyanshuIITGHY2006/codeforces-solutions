...
string to_binary(ll x, int width = 0) {
    string s;
    if (x == 0) s = "0";
    while (x > 0) { s += char('0' + (x & 1)); x >>= 1; }
    reverse(all(s));
    while ((int)s.size() < width) s = "0" + s;
    return s;}
void solve() {
    int x, y; r(x, y); swap(x,y); if(x==0) {o(x+y, 0); return;}int g = x+y; string s = to_binary(g);
    string hi = to_binary(x);
    int mini = INF;
    string temp = ""; temp.append(sz(hi), '0');
    for(int i = 0; i < sz(hi); i++)
    {
        if(hi[i]=='1' && s[sz(s) - sz(hi) + i]=='0')
        {
            break; 
        }
        else if(hi[i]=='0' && s[sz(s) - sz(hi) + i]=='1'){
            string check = temp;
            check[i] = '1'; 
            
            int yo = stoll(check, nullptr, 2);
            mini = min(mini, yo-x);
        }
        else if(hi[i]=='1')
        {
            temp[i] = '1';
            int yo = stoll(temp, nullptr, 2);
            if(yo==x) mini = min(mini, yo-x);
        }
        temp[i] = hi[i];
    }
    temp =""; temp.append(sz(s), '0');
    for(int i = sz(s) - sz(hi) - 1; i >= 0; i--) {
        if(s[i] == '1'){
            temp[i] = '1';
            int yo = stoll(temp, nullptr, 2);
            mini = min(mini, yo - x);
