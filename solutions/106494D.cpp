...
void solve()
{
    ...
    if (a.back() >= 0 && a[0] < 0)
    {
        ...
        {
            // if (n - i - 1 % 2 == 1)
            //     answer = max(a[i] * 2 + b[n - (n - i - 1) / 2 - 1] + b[n - (n - i) / 2 - 1], answer);
            // else
            // {
            //     answer = max(a[i] * 2 + b[n - (n - i - 1) / 2 - 1] + b[n - (n - i) / 2 - 2], answer);
            // }
            answer = max(a[i] * 2 + b[i + ((n - i) / 2)] + b[i + ((n - i + 1) / 2)], answer);
        }
        answer = max(answer, b[((n) / 2)] + b[((n - 1) / 2)]);
        o(answer);
    }
    else if (a.back() >= 0)
    {
        o(accumulate(all(a), 0ll));
    }
    else
    {
        o(a[((n) / 2)] + a[((n - 1) / 2)]);
