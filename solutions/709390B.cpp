...
void solve()
{
    ...
    rep(i, 0, n)
    {
        int first_multiples = a[i].se / p - (a[i].fi - 1) / p;
        int second_multiples = a[i+1].se / p - (a[i+1].fi - 1) / p;
        
        int total_first = a[i].se - a[i].fi + 1;
        int total_second = a[i+1].se - a[i+1].fi + 1;
        
        ld p1 = (ld)first_multiples / total_first;
        ld p2 = (ld)second_multiples / total_second;
