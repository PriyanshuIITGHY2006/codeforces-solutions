...
void solve() {
    ...
    nodes.pb(nodes.back());
    rev(nodes); nodes.pb(0); rev(nodes);
    dbgm(index); dbgv(nodes);
    rep(i, 0, n)
    {
        int start = index[a[i].fi];
        int end = index[a[i].se+1];
        if(nodes[end-1]-nodes[start-1]>0) continue;
        else
        {
            o(i+1);
            return;
