...
void solve()
{
    ...
    while (q--)
    {
        ...
        r(li, ri, k);
        bool possible = true;
        if (indexes[k] > ri || indexes[k] < li)
        {
            possible = false;
        }
        int need_maxi = 0, need_mini = 0;
        int temp = 0;
        int use_maxi =0, use_mini=0;
        if (possible)
        {
            while ((li + ri) / 2 != indexes[k])
            {
                int mid = (li + ri) / 2;
                if (indexes[k] < mid)
                {
            ...
            else if (k - use_mini < 1)
            {
                possible = false;
            }
            else
            {
                temp += 2 * min(need_maxi, need_mini);
                need_maxi -= (temp / 2);
                need_mini -= (temp / 2);
                temp += ((need_maxi * 2) + (need_mini * 2));
            }
        }
        if (possible)
            cout<<temp<<" ";
