...
void solve()
{
    ...
    vi a = rvec(n);
    vi b = rvec(m);
    int asum = accumulate(all(a), 0ll), bsum = accumulate(all(b), 0ll);
    set<int> ak, bk;
    rep(i, 0, n)
    {
        ak.insert(asum - a[i]);
    ...
    while (q--)
    {
        ...
        if (x < 0)
            sign = 0;
        int n = abs(x);
        for (int i = 1; (ll)i * i <= abs(x); i++)
        {
            if (abs(x) % i == 0)
            {
                if (sign == 0)
                {
                    if (ak.find(-i) != ak.end() && bk.find(n / i) != bk.end())
                    {
                        possible = 1;
                        break;
                    }
                    else if(ak.find(i) != ak.end() && bk.find(-(n / i)) != bk.end())
                    {
                        possible = 1;
                        break;
                    }
                    else if (bk.find(-i) != bk.end() && ak.find(n / i) != ak.end())
                    {
                        possible = 1;
                        break;
                    }
        ...
        if(x==0)
        {
            if(ak.find(0)!=ak.end() || bk.find(0)!=bk.end()){
                possible = 1;
            }
        }
        if (possible)
