...
void solve() {
    ...
    int value = accumulate(all(yo), 0ll);
    int siza = sz(yo); rev(yo);
    vi ans; ans.pb(value);
    int cnt = 0;
    // pv(yo);
    rep(i, 0, sz(s))
    {
        if(s[i]=='1'){
            if(siza==0) ans.pb(0);
                ...
                value-=(yo.back()-cnt); yo.pop_back();ans.pb(value); siza--;
            }
        }
        else{
            if(siza==0) ans.pb(0);
                ...
                cnt++;
                int buffer=0;
                rep(i, sz(yo)-siza, sz(yo))
                {
                    if(yo[i]-cnt<=0) buffer++;
                    else break;
                }
                value-=siza;
                ans.pb(value);
