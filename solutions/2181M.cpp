...
void solve() {
    ...
    int t=s1.size();
    vvi dp(2,vi(t));
    //dp[i][0]=minimum shifts needed to fix the rest of the string if we choose to flip s1
    //dp[i][1]=minimum shifts needed to fix the rest of the string if we choose to flip s2
    dp[0][0]=(s1[0] != '0') + (s2[0] != '0');
    dp[1][1]=(s1[0] != '1') + (s2[0] != '1');
    for(int i=1; i<t; i++)
    {
        // dp[]
    }
