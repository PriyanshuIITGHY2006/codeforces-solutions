...
void solve() {
    ...
    for(auto& it: a)
    {
        if(it.first==ax)
        {
            ...
            {
                it1.second=abs(it1.first-ay);
            }
        }
        else
        {
            ll mn = it.second.begin()->first;
            ll mx = prev(it.second.end())->first;
            
            auto prev_mn = *a[previous].begin();
            auto prev_mx = *prev(a[previous].end());
            
            for(auto& it1: it.second)
            {
                ll k= INF;
                for(auto it2 : {prev_mn, prev_mx})
                {
                    ll path1 = abs(it2.first - mx) + (mx - mn) + abs(mn - it1.first);
                    ll path2 = abs(it2.first - mn) + (mx - mn) + abs(mx - it1.first);
                    k = min(k, it2.second + it.first - previous + min(path1, path2));
                }
                it1.second=k;
            }
        }
