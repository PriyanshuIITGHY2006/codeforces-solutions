...
void solve() {
    ...
    vi visited(n+1,0);
    vi colour(n+1,-1);
    auto dfs= [&](auto && self, int node, int number, pii &k, int & is_posssible) -> pii{
        if(number==0) k.first=k.first+1;
        ...
        for(auto it: adj[node])
        {
            if(!visited[it])
            {
                self(self, it, number^1, k, is_posssible);
            }
            else
            {
                if(colour[it]!=colour[node]^1)
                {
                    is_posssible=0;
                }
            }
    ...
    for(int i=1; i<n+1; i++)
    {
        if(!visited[i])
        {
            pii k={0,0};
            int bipartitee = 1;
            dfs(dfs, i, 0, k, bipartitee);
            if(bipartitee)
            {
                ll to_add = (binpow(2, k.first) + binpow (2, k.second))%MOD;
