...
void solve()
{
    ...
    rep(j, 0, c)
    {
        ...
        {
            if(rows[i]>=3 && a[i][j]=='.')
            ans += (max(0ll, ((rows[i] - 1) * (rows[i] - 2))));
        }
        ways += 2 * (ans * (columns[j] - 1));
    }
    rep(i, 0, ri)
    {
        int ans = 0;
        ...
        rep(j, 0, c)
        {
            if(columns[j]>=3 && a[i][j]=='.')
            ans += (max(0ll, (((columns[j] - 1) * (columns[j] - 2)))));
        }
        ways += 2 * (ans * (rows[i] - 1));
    }
    rep(j, 0, c)
    {
        ...
        rep(i, 0, ri)
        {
            ...
            {
                sum1 += (rows[i] - 1);
                sum2 += ((rows[i] - 1) * (rows[i] - 1));
            }
        }
        int g = ((sum1 * sum1) - sum2);
        ways += g;
    }
    rep(i, 0, ri)
    {
        ...
        rep(j, 0, c)
        {
            if (columns[j] >= 2 && a[i][j]=='.')
            {
                sum1 += (columns[j] - 1);
                sum2 += ((columns[j] - 1) * (columns[j] - 1));
            }
        }
        int g = ((sum1 * sum1) - sum2);
        ways += g;
    ...
    rep(i, 0, ri)
    {
        if (rows[i] >= 4)
        {
            ways += (rows[i] * (rows[i] - 1) * (rows[i] - 2) * (rows[i] - 3));
        }
    }
    rep(j, 0, c)
    {
        if (columns[j] >= 4)
        {
            ways += (columns[j] * (columns[j] - 1) * (columns[j] - 2) * (columns[j] - 3));
        }
    }
    o(ways);
}
