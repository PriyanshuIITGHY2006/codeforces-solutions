...
void solve()
{
    ...
    r(n);
    vi a = rvec(n);
    map<int, int> primes, primesquares;
    map<int, pair<int, pii>> twoprimes;
    rep(i, 0, n)
    {
        map<int, int> k = prime_factorization(a[i]);
        int count = 0;
        int difference=0;
        for (auto it : k)
        {
        ...
        {
            if (twoprimes[a[i]].fi == 0)
            {
                twoprimes[a[i]] = {1,{(*k.begin()).fi, (*next(k.begin())).fi}};
            }
            else twoprimes[a[i]].fi++;
        }
        else if (count == 2)
        {
            primesquares[a[i]]++;
    ...
    dbgm(twoprimes);
    int pairs = 0;
    for(auto it : twoprimes)
    {
        if(it.se.fi!=0)
        {
            pairs += calc(it.se.fi);
            pairs += it.se.fi;
            pairs += (it.se.fi * (primes[it.se.se.fi]+primes[it.se.se.se]));
        }
    }
    dbg(pairs);
    for(auto it : primesquares)
    {
        if(it.se!=0)
        {
            pairs += calc(it.se);
            pairs+=it.se;
            pairs += (it.se * (primes[(int)sqrt(it.fi)]));
        }
    }
    int g = 0;
    for(auto it : primes)
