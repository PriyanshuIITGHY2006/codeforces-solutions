...
void dfs(map<int, vi> &m, int parent, int current, vi & level)
{
    if(parent==-1) level[current]=0; else level[current] = level[parent]+1;
    for(auto it : m[current])
    {
        if(it!=parent) dfs(m, current, it, level);
    }
}
...
void solve() {
    ...
    ll answer=0;
    for(auto it: t) answer+=(it.second%2);
    cout<<answer;
}
