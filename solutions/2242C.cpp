...
void solve() {
    ...
    for(auto it : volla)
    {
        hulle.pb({it.fi, it.se});
    }
    int cur_freq = 0;
    int cur_numbers = 0;
    vpii possible; possible.pb({0, 0});
    int level = 0;
    rep(i, 0, sz(hulle))
    {
        if(i!=sz(hulle)-1)
        {
            int g = hulle[i].fi-hulle[i+1].fi;
            cur_numbers+=hulle[i].se;
            level += cur_numbers;
            possible.pb({level, cur_numbers});
            g--;
            while(g--)
            {
                level += cur_numbers;
    ...
    rep(i, 1, sz(possible))
    {
        if((k-possible[i].fi)>=0)
        if((k-possible[i].fi)%possible[i].se==0)
        {
            cur++;
        }
    }
