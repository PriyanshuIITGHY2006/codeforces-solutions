...
void solve() {
    ...
    rep(i, 0, sz(g))
    {
        if(count(all(s), g[i])%(i+1)!=0)
        {
            o(-1); return;
        }
        yo[i] = count(all(s), g[i])/(i+1);
        rep(j, 0, sz(s))
        {
            if(yo[i]==0) break;
            if(s[j]== g[i]) {yo[i]--; max_index=max(max_index, j);}
        }
    }
    string ref = "";
    rep(i, 0, max_index+1)
    ...
    string ansi = ref;
    string answer = ""; answer.append(ref);
    rep(i, 0, sz(g))
    {
        string yo="";
        rep(j, 0, sz(ref))
        {
            if(ref[j]!=g[i]) yo.pb(ref[j]);
        }
        ref = yo;
        answer.append(ref);
    }
    if(answer!=s) {o(-1); return;}
    o(ansi, g);
}
