...
void solve()
{
    ...
    int qmax = 0, pmax = 0;
    int p = a[0], q = b[0];
    cout << (binpow(2, a[0]) + binpow(2, b[0])) % MOD << " ";
    rep(i, 1, n)
    {
        if (a[i] > p)
        {
        ...
        if (a[pmax] > b[qmax])
        {
            cout << (binpow(2, a[pmax]) + binpow(2, b[i - pmax])) % MOD << " ";
        }
        else if (a[pmax] < b[qmax])
        {
            cout << (binpow(2, a[i - qmax]) + binpow(2, b[qmax])) % MOD << " ";
        }
        else
        {
            if (b[i - pmax] >= a[i - qmax])
            {
                cout << (binpow(2, a[pmax]) + binpow(2, b[i - pmax])) % MOD << " ";
            }
            else
