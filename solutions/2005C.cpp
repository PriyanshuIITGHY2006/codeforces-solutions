...
void solve()
{
    ...
    vs a(n);
    rep(i, 0, n) r(a[i]);
    vector<vpii> dp(n + 1, vpii(5, {-INF, INF}));
    map<char, char> m;
    m['n'] = 'a';
    m['a'] = 'r';
    m['r'] = 'e';
    ...
    rep(i, 0, n)
    {
        freq[i] = count(all(a[i]), 'n') + count(all(a[i]), 'a') + count(all(a[i]), 'r') + count(all(a[i]), 'e') + count(all(a[i]), 'k');
    }
    dp[0][0] = {0, 0};
    rep(i, 1, n + 1)
    {
        rep(j, 0, 5)
        {
            ...
            if (dp[i - 1][j].fi != -INF)
            {
                if (dp[i][o].fi - dp[i][o].se <= dp[i - 1][j].fi + length - (dp[i - 1][j].se + freq[i - 1] - length))
                {
                    dp[i][o] = {dp[i - 1][j].fi + length, dp[i - 1][j].se + (freq[i - 1] - length)};
                }
            }
        }
        rep(l, 0, 5)
        {
            if(dp[i][l].fi - dp[i][l].se < dp[i-1][l].fi-dp[i-1][l].se) dp[i][l] = dp[i-1][l];
        }
    ...
    rep(i, 0, 5)
    {
        answer = max(answer, ((dp[n][i].fi-i)/5)*5 - dp[n][i].se - i);
    }
    o(answer);
}
