...
#define NO cout<< "NO\n";
void solve() {
    ll n, k, p, q; r(n, k, p, q);
    vll a = rvec(n);
    vll pi(n), qi(n), diffp(n), diffq(n);
    ll answer=0;
    for(int i=0; i<n;i++)
    {
        pi[i] = (a[i]%p)%q;
        qi[i] = (a[i]%q)%p;
        answer+=min(pi[i], qi[i]);
        diffp[i] = max(0LL, pi[i]-qi[i]); //if q chosen difference from best answer
        diffq[i] = max(0LL, qi[i]-pi[i]); // if p chosen diff from best
    }
    vll available_swaps;
    ll pd=0, qd=0;
    for(int i=0; i<k; i++)
    ...
    available_swaps.pb(min(pd, qd));
    for(int i=k; i<n; i++)
    {
        pd -= (diffp[i-k]-diffp[i]);
        qd -= (diffq[i-k]-diffq[i]);
        available_swaps.pb(min(pd, qd));
    }
    srt(available_swaps);
    ll problem=available_swaps[0];
    o(answer+problem);
}
