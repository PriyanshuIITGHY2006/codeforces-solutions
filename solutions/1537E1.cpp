...
#define NO cout<< "NO\n";
void solve() {
    int n, k; r(n, k); string s; r(s);
    vs a(n); string temp = "";
    for(int i=0; i<n; i++)
    {
        temp.pb(s[i]); a[i]=temp;
    }
    temp.clear();
    vi indexes;
    int temp_index = n-1;
    for(int i=1; i<n; i++)
    {
        if(s[i]>s[0]) {temp_index=i-1; break;}
        else if (s[i]==s[0]) indexes.pb(i);
    }
    string s1 = s+s;
    for(auto it: indexes)
    {
    ...
    string ansi = "";
    for(int i=0; i<=min(k-1,temp_index); i++)
    {
        ansi.pb(s[i]);
    }
    int g = k/sz(ansi);
    string answer ="";
    for(int i=0; i<g; i++) answer = answer + ansi;
    int h = k%sz(ansi);
    for(int i=0; i<h; i++) answer.pb(s[i]);
    o(answer);
