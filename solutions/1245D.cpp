...
struct DSU {
    ...
    bool unite(int i, int j) {
        i = find(i); j = find(j);
        if (i == j) return false;
        if (rank_[i] < rank_[j]) swap(i, j);
        parent[j] = i;
        if (rank_[i] == rank_[j]) rank_[i]++;
        return true;
    }
    bool same(int i, int j) { return find(i) == find(j); }
    int size(int i) { return rank_[find(i)]; }
};
void solve() {
    int n;
    r(n);
    vpii a(n+1);
    rep(i, 1, n+1) rp(a[i]);
    vi c = rvec(n); vi k = rvec(n);
    vvi edges;
    rep(i, 1, n+1){
        rep(j, i+1, n+1)
        {
            vi temp(3);
            temp[0] = (k[i-1] + k[j-1])*(abs(a[i].fi-a[j].fi)+abs(a[i].se-a[j].se));
            temp[1] = i;
            temp[2] = j;
            edges.pb(temp);
        }
    ...
    for (auto w : edges) {
        if (dsu.unite(w[2], w[1])) {
            mst_cost += w[0];
            mst_edges.push_back({w[1], w[2]});
        }
    }
    o(mst_cost);
    vi power;
    for(auto it : mst_edges){
        if(it.fi==n+1 || it.se==n+1) power.pb(min(it.fi, it.se));
    }
    o(sz(power)); srt(power); pv(power); o(sz(mst_edges)-sz(power));
    for(auto it : mst_edges){
        if(it.fi==n+1 || it.se==n+1) continue;
        pp(it);
    }
}
