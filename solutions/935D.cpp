...
void solve()
{
    ...
    vi b = rvec(n);
    // int index = n - 1;
    if ((count(all(a), 0) + count(all(b), 0)) == 0)
    {
        rep(i, 0, n)
        {
            if(a[i]>b[i])
    ...
    rep(i, 0, n)
    {
        ...
        {
            int g = m - b[i];
            probab = (probab + (cur * ((g * modinv(m)) % MOD)) % MOD) % MOD;
            cur = (cur * modinv(m)) % MOD;
        }
        else if (a[i] != 0 && b[i] == 0)
        {
            int g = a[i] - 1;
        ...
        {
            int k = ((m * (m - 1)) / 2) % MOD;
            probab = (probab + (cur * ((((k * modinv(m)) % MOD) * modinv(m)) % MOD) % MOD)) % MOD;
            cur = ((((((cur * modinv(m)) % MOD) * modinv(m)) % MOD)) * m) % MOD;
        }
        else if (a[i] > b[i])
        {
            probab = (probab + cur) % MOD;
            break;
        }
        else
            break;
