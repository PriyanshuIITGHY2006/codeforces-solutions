...
void solve() {
    ...
    dfs(1);
    if(!visited[n]) {o(-1);return;}
    vpll djikstra(n+1,{INF, -1});
    djikstra[1]={0,-1};
    priority_queue<pll, vector<pll>, greater<pll>> pq;
    pq.push({0,1});
    while (!pq.empty()) {
        auto [d, u] = pq.top(); pq.pop();
        if (d > djikstra[u].fi) continue;
        for (auto [v, w] : adj[u]) {
            if (djikstra[u].fi + w < djikstra[v].fi) {
                djikstra[v] = {djikstra[u].fi + w, u};
                pq.push({djikstra[v].fi, v});
            }
        }
    }
    vi answer;
