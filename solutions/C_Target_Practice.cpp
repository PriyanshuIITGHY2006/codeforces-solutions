...
// ═══════════════════════════════════════════════════════
#ifdef DEBUG
#define DC  "\033[36m"   // cyan
#define DY  "\033[33m"   // yellow
#define DR  "\033[31m"   // red
#define DG  "\033[32m"   // green
#define DX  "\033[0m"    // reset
...
void solve() {
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 10; j++) {
            ...
            dbg(cell);
            if (cell == 'X') {
                int dist_to_edge = min({i, 9 - i, j, 9 - j});
                int points = dist_to_edge + 1;
                total_points += points;
            }
        }
    }
