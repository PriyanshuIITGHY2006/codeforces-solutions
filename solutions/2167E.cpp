...
void solve()
{
    ...
    rep(i, 0, sz(a) - 1)
    {
        if ((a[i + 1] - a[i]) % 2 == 0)
        {
            g.pb({((a[i] + a[i + 1]) / 2 - a[i]), ((a[i] + a[i + 1]) / 2)});
        }
        else
        {
            g.pb({((a[i] + a[i + 1]) / 2 - a[i]), (((a[i] + a[i + 1]) / 2) + 1)});
            g.pb({((a[i] + a[i + 1]) / 2 - a[i]), ((a[i] + a[i + 1]) / 2)});
        }
    }
    uni(g);
    ...
    while (cur >= 0)
    {
        ...
        rep(j, 0, i + 1)
        {
            if (((g[j].se - (g[j].fi - cur + 1)) >= 0) && (g[j].se - (g[j].fi - cur + 1) <= x))
            {
                ans.insert(g[j].se - (g[j].fi - cur + 1));
                if (sz(ans) == k)
                {
                    done = true;
                    break;
            ...
            if (((g[j].se + (g[j].fi - cur + 1)) <= x) && ((g[j].se + (g[j].fi - cur + 1)) >= 0))
            {
                ans.insert(g[j].se + (g[j].fi - cur + 1));
                if (sz(ans) == k)
                {
