...
void solve() {
    ...
    for(auto it : diffs)
    {
        for(auto it1 : diffs)
        {
            diffsum[it.fi + it1.fi] += (it.se*it1.se);
        }
    }
    vi voila, voila2;
    for(auto it : diffsum){
    ...
    for(auto it : diffs)
    {
        auto it1 = lower_bound(all(voila), it.fi);
        if(it1==voila.begin()) continue;
        it1--;
        int g = it1 - voila.begin();
        dbg(g);
        dbg(voila2[g]);
        probab = probab +  ((ld)it.se / denom1) * (ld) ((ld)voila2[g] / denom2);
    }
