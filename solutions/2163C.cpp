...
void solve() {
    ...
    vi minfromright1(n), maxfromright1(n), maxfromleft2(n), minfromleft2(n);
    minfromright1[n-1]=a[1][n-1];
    maxfromright1[n-1]=a[1][n-1];
    maxfromleft2[0]=a[0][0];
    minfromleft2[0]=a[0][0];
    ll higestl=1, lowestr=2*n;
    for(int i=1; i<n; i++)
    {
        minfromleft2[i]=min(minfromleft2[i-1], a[0][i]);
        maxfromleft2[i]=max(maxfromleft2[i-1], a[0][i]);
    }
    for(int i=n-2; i>=0; i--)
    {
        minfromright1[i]=min(minfromright1[i+1], a[1][i]);
        maxfromright1[i]=max(maxfromright1[i+1], a[1][i]);
    }
    vpii pairs_possible;
    for(int i=0; i<n; i++)
    {
        pairs_possible.pb(mp(min(minfromright1[i], minfromleft2[i]), max(maxfromright1[i], maxfromleft2[i])));
    }
    srt(pairs_possible);
    int k=0;
    ll answer=0;
    multiset<int> m;
    for(auto it:pairs_possible)
    m.insert(it.second);
    for(int i=1; i<2*n+1; i++)
    {
       while(k < pairs_possible.size() && pairs_possible[k].first < i) {
        m.erase(m.find(pairs_possible[k].second));
        k++;
    }
        if(!m.empty())
        answer+=(2LL*n-(*m.cbegin())+1);
    }
    cout<<answer<<"\n";
}
