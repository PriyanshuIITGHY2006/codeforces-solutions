...
void solve()
{
    int n;
    r(n);
    map<int, vector<tuple<int, int, ld>>> m;
    rep(i, 0, n)
    {
        int x, y, t;
        ld p;
    ...
    vector<tuple<int, int, int, ld>> dp;
    ld hi = -1;
    for (auto it : m)
    {
        vector<tuple<int, int, int, ld>> temp;
        for (auto [x, y, p] : it.se)
        {
            ld curp = 0;
            for (auto [x1, y1, t1, p1] : dp)
            {
                if (((x1 - x) * (x1 - x)) + ((y1 - y) * (y1 - y)) <= (it.fi - t1) * (it.fi - t1))
                {
                    curp = max(curp, p1);
                }
            }
            temp.eb(x, y, it.fi, curp + p);
            hi = max(hi, curp + p);
        }
        for (auto it1 : temp)
            dp.pb(it1);
