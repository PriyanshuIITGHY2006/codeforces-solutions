...
#define YES cout<< "YES\n";
#define NO cout<< "NO\n";
ll dpc(vvll &dp, vll &a, int i, int j)
{
...
{
    m[a[i]]++;
    if(!(a[i]==a[i-1] || a[i]==a[i-1]+1)) {mex_index=i-1; break;}
}
if(mex_index==-1) mex_index=n-1;
mex= a[mex_index];

//answer is dp[zero_count][mex_index]
// if k==0, a[mex_index]==0, then dp[][]=0, if zero_count==0, dp[][]=0, 
}
