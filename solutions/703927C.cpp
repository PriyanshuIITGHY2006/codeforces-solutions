...
void solve()
{
    ...
    string s;
    r(s);
    vi st(n+1), rs(n+1), lr(n+2), rr(n+2, 0);
    int p =0;
    rep(i, 1, n+1)
    {
        p^=(s[i-1]=='0'?1:2);
        st[i]=p;
        rs[i]= ((i==1)||s[i-1]!=s[i-2]) ? i : rs[i-1];
        lr[i] = i - rs[i] + 1;
    }
    per(i, 1, n+1) rr[i] = (i==n || s[i-1]!=s[i]) ? 1 : rr[i+1] + 1;
    int best[4] ={-1, -1, -1, -1}, ptr =0, m =0, clean =0;
    rep(R, 1, n+1)
    {
        while(ptr<=rs[R]-2) {
            best[st[ptr]]=ptr; ptr++;
        }
        if(best[st[R]]>=0) m = max(m, best[st[R]]+1);
        clean += R-m;
    }
    int cs = 0;
    for(int i=1; i<=n;)
    ...
    rep(i, 1, n)
    {
        if(s[i-1]=='0' && s[i]=='1') cs +=lr[i]+rr[i+1]-1;
    }
    o(n*(n+1)/2 - clean + cs);
}
