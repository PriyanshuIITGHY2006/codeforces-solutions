...
#define YES cout<< "YES\n";
#define NO cout<< "NO\n";
bool check(ll n, vll &a, vll &b) {
    ll currc = 0; ll currt = 0;
    ll minc = b[0]-n;
    ll maxc = b[0]+n;
    while(currc<sz(a) && currt<sz(b))
    {
        minc = b[currt] - n; maxc = b[currt] + n;
        if(a[currc]<=maxc && a[currc]>=minc)
        {
            currc++;
        }
        else
