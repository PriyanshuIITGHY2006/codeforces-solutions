...
void solve() {
    ...
    auto dfs = [&](auto && self, pii x, pii y, pii z, bool & cycle, char c, vvi & visited)->void{
        if(x.fi>=n || x.fi<0 || x.se>=m || x.se<0) return;
        else if(x==z && y.fi!=-1)
        {
            cycle=true;
            return;
        }
        else if (a[x.fi][x.se]!=c)
        {
            return;
        }
        else if(visited[x.fi][x.se]==1)
        {
            return;
        }
        else
        {
            visited[x.fi][x.se]=1;
            pii x1={x.fi+1, x.se}, x2 = {x.fi, x.se+1}, x3 = {x.fi-1, x.se}, x4 ={x.fi, x.se-1};
            if(x1!=y) self(self, x1, x, z, cycle, c,visited);
            if(x2!=y) self(self, x2, x, z, cycle, c,visited);
            if(x3!=y) self(self, x3, x, z, cycle, c,visited);
            if(x4!=y) self(self, x4, x, z, cycle, c,visited);
            return;
        }
    };
    for(int i=0; i<n; i++)
    {
        for(int j=0; j<m; j++)
        {
            ...
            dfs(dfs, {i,j}, {-1,-1}, {i,j}, cycle, a[i][j], visited);
            if(cycle)
            {
                YES return;
            }
