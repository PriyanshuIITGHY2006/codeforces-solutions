...
vector<vector<int>> strongly_connected_components(vector<vector<int>> const& adj) {
    int n = adj.size() - 1; 
...
void solve() {
    ...
    vvi components = strongly_connected_components(adj);
    int answer1 = 0, answer2 = 1;
    rep(i, 0, sz(components))
    {
        rep(j, 0, sz(components[i])){
            components[i][j]= a[components[i][j]-1];
        }
        int g = mn(components[i]);
        int c = count(all(components[i]), g);
        answer1 += g;
        answer2 = (answer2*c)%MOD;
    }
    o(answer1, answer2);
}
