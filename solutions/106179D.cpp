...
void solve()
{
    ...
    rep(i, 1, n + 1)
    {
        auto it = lower_bound(all(left), i);
        auto it1 = upper_bound(all(right), i);
        if (it == left.end())
            it--;
        if ((*it) > i)
        {
            if (it == left.begin() && it1 == right.end())
            {
                o(1);
                cout << n << " ";
                vpii temp;
                for (auto it11 : left)
                    temp.pb({index[it11], it11});
                for (auto it11 : right)
                    temp.pb({index[it11], it11});
                srt(temp);
                rep(i, 0, n) cout << temp[i].se << " ";
                nl;
                return;
            }
        ...
        int first = it - left.begin();
        int second = right.end() - 1 - it1;
        if (first != second)
            continue;
        else
        {
            ...
            rep(j, 0, first + 1)
            {
                temp1.pb({index[left[j]], left[j]});
            }
            srt(temp1);
            rep(i, 0, sz(temp1)) cout << temp1[i].se << " ";
            temp1.clear();
            rep(j, n / 2 - second - 1, (n / 2))
            {
                temp1.pb({index[right[j]], right[j]});
            }
            srt(temp1);
            rep(i, 0, sz(temp1)) cout << temp1[i].se << " ";
            temp1.clear();
