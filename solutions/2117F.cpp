...
void solve()
{
    ...
    function<void(int)> dfs = [&](int u)
    {
        ...
        if (u == 1 && sz(adj[u]) > 1)
        {
            splitting += (sz(adj[u]) - 1);
            split_point = u;
        }
        else if (sz(adj[u]) > 2)
        {
            splitting += (sz(adj[u]) - 2);
            split_point = u;
        }
        for (int v : adj[u])
        {
    ...
    else if (splitting == 1)
    {
        ...
        dfs(1, 0);
        vi temp;
        for(auto it : adj[split_point])
        {
            if(sub[it]<sub[split_point]) temp.pb(sub[it]);
        }
        int a = temp[0] , b = temp[1]; if(a>b) swap(a, b);
        if(a==b)
        {
            o(binpow(2, n-a-b+1)); return;
        }
        else
        {
            o((binpow(2, n-a-b)*(binpow(2, b-min(a,b)) + binpow(2, b-(min(a,b)+1))))%MOD);
        }
    }
    else
    {
