...
void solve()
{
    ...
    rep(j, 1, n)
    {
        rep(m, 0, n + 1)
        {
            rep(k, 0, 51)
            {
                if (s[j] == '(')
                {
                    dp[j][m][k][0] = (dp[j - 1][m][k][0] + ((k != 0 && m!=0) ? dp[j - 1][m-1][k - 1][0] : 0))%MOD;
                    dp[j][m][k][1] = dp[j - 1][m][k][1];
                    dp[j][m][k][2] = (dp[j - 1][m][k][2] + ((k != 0 && m!=0) ? dp[j - 1][m-1][k - 1][1] : 0))%MOD;
                    dp[j][m][k][3] = (dp[j - 1][m][k][3] + ((k != 0 && m!=0) ? (dp[j - 1][m-1][k - 1][2] + dp[j - 1][m-1][k - 1][3]) : 0))%MOD;
                }
                else
                {
                    dp[j][m][k][0] = dp[j - 1][m][k][0];
                    dp[j][m][k][1] = (dp[j - 1][m][k][1] + ((k != 50 && m!=0) ? (dp[j - 1][m-1][k + 1][0] + dp[j - 1][m-1][k + 1][1]) : 0))%MOD;
                    dp[j][m][k][2] = (dp[j - 1][m][k][2] + ((k != 50 && m!=0) ? dp[j - 1][m-1][k + 1][2] : 0))%MOD;
                    dp[j][m][k][3] = (dp[j - 1][m][k][3] + ((k != 50 && m!=0) ? dp[j - 1][m-1][k + 1][3] : 0))%MOD;
                }
            }
        }
    }
    ...
    // rep(i, 0, n){
        rep(j, 6, n+1)
        {
            answer = (answer + (dp[n-1][j][0][3]*(j-2))%MOD)%MOD;
        }
    // }
    o(answer);
}
