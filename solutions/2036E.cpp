...
void solve()
{
    ...
    vvi containers(k, vi(n));
    rep(j, 0, k)
    {
        b[0][j] = a[0][j];
        containers[j][0] = b[0][j];
        rep(i, 1, n)
        {
            b[i][j] = b[i - 1][j] | a[i][j];
            containers[j][i] = b[i][j];
        }
    }
    vi g(n, 0);
    // pv2(containers);
    while (q--)
    {
        ...
        while (m--)
        {
            ...
            if (o == '<')
            {
                auto it = lower_bound(all(containers[ri - 1]), c);
                if (it != containers[ri - 1].begin())
                {
                    it--;
                    int offset = it - containers[ri - 1].begin();
                    dbg3(m, o, offset);
                    if(offset<mini)
                    {
                        mini = -2; 
                    }
                    else if(mini!=-2) {maxi = min(maxi, offset);}
                }
                else {
                    mini = -2;
                }
            ...
            {
                auto it = upper_bound(all(containers[ri - 1]), c);
                if (it != containers[ri - 1].end())
                {
                    int offset = it - containers[ri - 1].begin();
                    dbg3(m, o, offset);
                    if(offset>maxi)
                    ...
                    else if(mini!=-2) mini =max(offset, mini);
                }
                else {
                    mini =  -2; 
                }
